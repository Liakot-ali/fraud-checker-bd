...
// Convert Bengali (০–৯) and Arabic-Indic (٠–٩) digits to ASCII so the regex
// extractors work on Bengali posts and NID cards, not only English ones.
function toAsciiDigits(s) {
    return String(s == null ? '' : s).replace(/[০-৯٠-٩]/g, (d) => {
        const c = d.codePointAt(0);
        if (c >= 0x09E6 && c <= 0x09EF) return String(c - 0x09E6); // Bengali
        if (c >= 0x0660 && c <= 0x0669) return String(c - 0x0660); // Arabic-Indic
        return d;
    });
}

// A Bangladeshi mobile number, tolerant of the separators people actually type:
// "017 1234 5678", "017-1234-5678", "+880 1712-345678". Digits are re-joined and
// the country code collapsed to a leading 0 by normBdPhone().
const BD_PHONE_RE = /(?:\+?880[\s-]?|0)1[3-9](?:[\s-]?\d){8}/g;
function normBdPhone(s) {
    const d = String(s == null ? '' : s).replace(/\D/g, '').replace(/^880/, '0');
    return /^01[3-9]\d{8}$/.test(d) ? d : '';
}

function extractFromText(text) {
    const t = toAsciiDigits(String(text == null ? '' : text));
    const uniq = (arr) => Array.from(new Set(arr));
    const phones = uniq((t.match(BD_PHONE_RE) || []).map(normBdPhone).filter(Boolean));
...
// Extract labelled person fields (name / address) from OCR'd documents or pasted
// text. Best-effort: keys off the common labels on Bangladeshi NID cards and posts
// (English + Bengali, incl. the Bengali visarga "ঃ" often used as a colon).
function extractPerson(text) {
    const t = String(text == null ? '' : text);
    const uniq = (a) => Array.from(new Set(a.map((s) => s.trim()).filter(Boolean)));
    const names = [];
    const addresses = [];
    const sep = '\\s*(?::|：|ঃ|-)\\s*'; // colon / fullwidth colon / Bengali visarga / hyphen
    const nameRe = new RegExp('(?:^|[\\s>|])(?:name|full name|name of person|নাম)' + sep + '(.+)$', 'i');
    const addrRe = new RegExp('(?:^|[\\s>|])(?:address|present address|permanent address|ঠিকানা|বর্তমান ঠিকানা|স্থায়ী ঠিকানা)' + sep + '(.+)$', 'i');
    for (const raw of t.split(/\r?\n/)) {
        const line = raw.trim();
        const nm = line.match(nameRe);
        if (nm) { const v = nm[1].trim(); if (v.length >= 2 && v.length <= 120) names.push(v); }
        const ad = line.match(addrRe);
        if (ad) { const v = ad[1].trim(); if (v.length >= 3) addresses.push(v.slice(0, 300)); }
    }
    return { names: uniq(names), addresses: uniq(addresses) };
}

// Context-aware money-trail extraction for the OCR/regex fallback. Finds the MFS
// provider and — crucially — the RECEIVING wallet number (a phone that sits on a
// line mentioning a provider or a "send money / cash out" cue), so the wallet is
// routed to the money field instead of polluting the accused's phone list.
const MFS_MATCHERS = [
    ['bKash', /b[\s-]?kash|বিকাশ/i],
    ['Nagad', /\bnagad\b|নগদ/i],
    ['Rocket', /\brocket\b|রকেট/i],
    ['Upay', /\bupay\b|উপায়/i],
    ['mCash', /\bmcash\b/i],
    ['SureCash', /sure[\s-]?cash/i],
    ['Tap', /\btap\b/i]
];
const MFS_CUE_RE = /b[\s-]?kash|nagad|rocket|upay|mcash|surecash|send\s*money|cash[\s-]?out|personal|received|payment|বিকাশ|নগদ|রকেট|সেন্ড|পাঠিয়েছি|পাঠাই|টাকা\s*পাঠ/i;
// Every BD phone in the text with its character offset (fresh regex to avoid any
// shared lastIndex state on the global BD_PHONE_RE).
function bdPhonesWithIndex(t) {
    const out = []; let m; const re = new RegExp(BD_PHONE_RE.source, 'g');
    while ((m = re.exec(t)) !== null) { const p = normBdPhone(m[0]); if (p) out.push({ p, index: m.index }); }
    return out;
}
// Pick the phone nearest to a cue word, strongly preferring one that appears AFTER
// the cue ("bKash … <wallet>", "my number is <phone>"). This is far more reliable
// than a line-scan when the whole story is written on a single run-on line.
function nearestPhoneToCue(t, cueRe) {
    const cue = t.search(cueRe);
    if (cue < 0) return '';
    let best = '', bestScore = Infinity;
    for (const { p, index } of bdPhonesWithIndex(t)) {
        const score = index >= cue ? (index - cue) : (cue - index) + 1000; // after-cue wins
        if (score < bestScore) { bestScore = score; best = p; }
    }
    return best;
}
function extractMoneyTrail(text) {
    const t = toAsciiDigits(String(text == null ? '' : text));
    let provider = '';
    for (const [name, re] of MFS_MATCHERS) { if (re.test(t)) { provider = name; break; } }
    const wallet = nearestPhoneToCue(t, MFS_CUE_RE);
    const { trxids } = extractFromText(t);
    return { provider, wallet, trxid: trxids[0] || '' };
}

// Detect the REPORTER's own number from first-person cues ("my number", "amar
// number", "আমার নম্বর") so the OCR fallback can keep it out of the accused's
// phone list (and optionally pre-fill the reporter's contact field).
const REPORTER_CUE_RE = /\bmy\s*(?:number|mobile|phone|cell|contact|no\.?)\b|\bamar\s*(?:number|nombor|nomber|phone|mobile)\b|আমার\s*(?:নাম্বার|নম্বর|মোবাইল|ফোন|যোগাযোগ)/i;
function extractReporterContact(text) {
    const t = toAsciiDigits(String(text == null ? '' : text));
    return { phone: nearestPhoneToCue(t, REPORTER_CUE_RE) };
}
...
module.exports = {
    ...
    extractMoneyTrail, extractReporterContact, normBdPhone,
